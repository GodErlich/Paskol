...
function setup() {
  ...
  let canvasX = (windowWidth - width) / 2;  // X offset from left
  let canvasY = (windowHeight - height) / 2; // Y offset from top
  canvas.position(canvasX, canvasY);
  
  fft = new p5.FFT(0.8, 1024);
  
  setupControls(canvasX, canvasY);
}
...
function setupControls(canvasX, canvasY) {
  ...
  let startX = canvasX + (width - totalWidth) / 2;
  let buttonsY = height + canvasY + 80;
  
  let philharmonicBtn = createButton('Israel Philharmonic Orchestra');
  philharmonicBtn.position(startX, buttonsY);
  philharmonicBtn.size(buttonWidth);
  philharmonicBtn.mousePressed(() => toggleMusic('philharmonic'));
  
  let markBtn = createButton('Mark Eliyahu');
  markBtn.position(startX + buttonWidth + spacing, buttonsY);
  markBtn.size(buttonWidth);
  markBtn.mousePressed(() => toggleMusic('mark'));
  
  let shuzinBtn = createButton('Shuzin');
  shuzinBtn.position(startX + (buttonWidth + spacing) * 2, buttonsY);
  shuzinBtn.size(buttonWidth);
  shuzinBtn.mousePressed(() => toggleMusic('shuzin'));

  // Style music buttons
  let buttons = selectAll('button:not(.arrow-button)');
...
function styleArrowButton(button) {
...
function updateButtonStyles() {
  ...
  if (currentlyPlaying) {
    buttons.forEach(button => {
      if (button.html().toLowerCase().includes(currentlyPlaying)) {
